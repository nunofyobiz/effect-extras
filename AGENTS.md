...
`docgen.json` turns on `enforceDescriptions`, `enforceExamples`, and `enforceVersion` for
`@effect/docgen`, so every exported symbol needs a description, an `@example` (type-checked under
`strict` + `exactOptionalPropertyTypes` — see [Verification ritual](#verification-ritual)), and an
`@since`. Every module file also opens with a header JSDoc block. Match the existing code's practice
of tagging each export `@category <group>` and `@since 0.0.0`. `src/index.ts` and `*.test.ts` files
are excluded from the enforcement. `/docs/examples` holds gitignored type-check scratch artifacts from
running the examples; the generated `docs/modules/*.ts.md` reference pages are committed and
regenerated by `pnpm docgen` (automated locally by the pre-commit hook — see
...
- `.agent-ops/` is gitignored — use it for agent scratch files that shouldn't be committed (e.g. a PR
  body passed to `gh pr create --body-file`).
